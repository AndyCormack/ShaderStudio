...
A bordered card the live preview fills edge to edge (on Viewport Black), with a round, gradual edge feather dissolving the render's outer ~40% into the card surface on every side, so any shader color blends into the chrome instead of ending at a hard rectangle. The selected-shader detail strip is a raised panel — a dark plum surface with a subtle top→bottom lift and a lighter purple edge (all sampled from the mockup) — inset within a full-window band, so it reads as distinct from the rail and command band rather than blending into them. Its tag chips are outlined in dimmed Signal Red (the focused shader's accent), distinguishing them from the neutral outlined chips on gallery tiles.

Title, outlined harness badge, mono fragment path, and outlined tag chips overlay the render's lower edge on a plain linear fade (Shadow Plum at the bottom → transparent at the top). Below them, a separate solid Shadow Plum footer bar carries the muted tabular meta row (GLSL version · source size · updated-ago) with a quiet ⋯ overflow menu at its right, divided from the content above by a subtle one-pixel top border (a faint divider when idle; a berry-magenta tint and border when the card is selected — both sampled from the mockup). The favorite star sits top-left over the preview, always visible: Muted Ink at rest, Favorite Rose filled when active. Selection is a dimmed Signal Red card border with a large, subtle Signal glow that hazes most of the card (edge-weighted, like the preview vignette), and its meta footer gains a berry-pink tinted background and border. Hot Coral never appears as tile chrome or status.
