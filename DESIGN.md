...
A bordered card the live preview fills edge to edge (on Viewport Black), with a round, gradual edge feather dissolving the render's outer ~40% into the card surface on every side, so any shader color blends into the chrome instead of ending at a hard rectangle. The selected-shader detail strip is a raised panel — lifted a step above the surface chrome with a lighter purple edge (both sampled from the mockup) — inset within a full-window band, so it reads as distinct from the rail and command band rather than blending into them.
