...
A magma sphere built from a single **multi-stop heat ramp**. Domain-warped **turbulence** (a sum of abs-value-noise — the "fractal cloud") defines the heat: where the turbulence dips, molten lava glows; the high folds stay dark cold rock. One `heat` scalar runs the whole surface through the ramp — cold rough rock (`u_crustColor`) → deep-red ember glow (`u_emberColor`) → orange → gold core (`u_coreColor`) → white-hot. A rough rock texture (ridged noise + grain + sharp pits) shades the cold end, and a world-space Fresnel term rings the silhouette. Hot cores and rim are emitted as HDR (>1) so the studio **bloom** pass ([D21]) gives them their glow.
