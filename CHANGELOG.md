...
### Added
- New **`select` uniform type** ([D23]): a labelled dropdown bound to a GLSL `int`, alongside `float`/`color`. Declared in `meta.json` with `options` (`{ value, label }`) + numeric `default`, rendered as a shadcn Select in the uniform panel. First use — the **Fireball rim** gained a **blend-mode** dropdown (Add · Screen · Overlay · Mix) and a **`u_rimPower`** float for the Fresnel falloff (how far the rim encroaches).
