...
### Added
- New **`select` uniform type** ([D23]): a labelled dropdown bound to a GLSL `int`, alongside `float`/`color`. Declared in `meta.json` with `options` (`{ value, label }`) + numeric `default`, rendered as a shadcn Select in the uniform panel. First use — the **Fireball rim** gained a **blend-mode** dropdown (the full Photoshop set — Normal, Add, Screen, Multiply, Overlay, Darken, Lighten, Color Dodge/Burn, Linear Burn/Light, Hard/Soft/Vivid/Pin Light, Hard Mix, Difference, Exclusion, Subtract, Divide) and a **`u_rimPower`** float for the Fresnel falloff (how far the rim encroaches). The rim composites as a masked layer (blend mode × Fresnel coverage), so a mode only acts along the silhouette.
...
### Changed
- Fireball bloom defaults widened to a soft ambient glow (radius `1`, threshold `0.25`); rim defaults to **Hard Light** blend at `u_rimPower` 5.5 (a tighter, hotter edge).
- Uniform-panel control rows drop the hover background-colour transition — the row highlight snaps in for a snappier feel.
...
- Gallery tiles gained a **hover outline** — a Signal-Red ring matching the selected border, drawn as an outline so it snaps in with no fade.
