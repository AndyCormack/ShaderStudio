...
- **Gallery previews:** the shared scissored single context ([D7]) can't render a composer straight to a tile region, so each post-fx tile composites **offscreen and unscissored** (its own keyed `EffectComposer` sized to the tile) and the result is **blitted** into the tile's scissored region via a fullscreen quad. Keeping the composite linear (no `OutputPass`) lets the blit encode sRGB once on the way to the canvas.
